...
class AddProdRazaoToSyncJob implements ShouldQueue
{
    ...
    {
        $importados = ImportXml::vendaItens($this->empresa, $this->filename);
        $this->empresa->arquivos()->where('nome', $this->filename)->update(['processado' => true]);

        $this->empresa->update(['ultima_sincronizacao' => now()]);
        $this->empresa->logs()->create([ 'log' => "[VENDAITENS] Foram importados {$importados}"]);
    }
}
