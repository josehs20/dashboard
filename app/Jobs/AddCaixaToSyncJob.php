...
class AddCaixaToSyncJob implements ShouldQueue
{
    ...
    {
        $importados = ImportXml::caixa($this->empresa, $this->filename);

        $this->empresa->arquivos()->where('nome', $this->filename)->update(['processado' => true]);

        $this->empresa->update(['ultima_sincronizacao' => now()]);
        $this->empresa->logs()->create(['log' => "[CAIXA] Foram importados {$importados}"]);
    }
}
