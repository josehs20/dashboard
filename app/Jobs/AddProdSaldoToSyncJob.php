...
class AddProdSaldoToSyncJob implements ShouldQueue
{
    ...
    {
        $importados = ImportXml::estoque($this->empresa, $this->filename);
        $this->empresa->arquivos()->where('nome', $this->filename)->update(['processado' => true]);

        $this->empresa->update(['ultima_sincronizacao' => now()]);
        $this->empresa->logs()->create([ 'log' => "[ESTOQUE] Foram importados {$importados}"]);
    }
}
