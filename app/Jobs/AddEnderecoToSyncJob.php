...
class AddEnderecoToSyncJob implements ShouldQueue
{
    ...
    {
        $importados = ImportXml::enderecos($this->empresa, $this->filename);
        $this->empresa->arquivos()->where('nome', $this->filename)->update(['processado' => true]);

        $this->empresa->update(['ultima_sincronizacao' => now()]);
        $this->empresa->logs()->create([ 'log' => "[ENDERECO] Foram importados {$importados}"]);
    }
}
