...
class AddPagarToSyncJob implements ShouldQueue
{
    ...
    {
        $importados = ImportXml::pagar($this->empresa, $this->filename);
        $this->empresa->arquivos()->where('nome', $this->filename)->update(['processado' => true]);

        $this->empresa->update(['ultima_sincronizacao' => now()]);
        $this->empresa->logs()->create([ 'log' => "[PAGAR] Foram importados {$importados}"]);
    }
}
