...
class AddIgradeToSyncJob implements ShouldQueue
{
    ...
    {
        $importados = ImportXml::iGrades($this->empresa, $this->filename);
        
        $this->empresa->arquivos()->where('nome', $this->filename)->update(['processado' => true]);

        $this->empresa->update(['ultima_sincronizacao' => now()]);
        $this->empresa->logs()->create([ 'log' => "[IGRADE] Foram importados {$importados}"]);
    }
}
